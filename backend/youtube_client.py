...
class YouTubeClient:
    ...
    def _execute_query(self, query_kind: str, query: str, **query_params) -> list:
        ...
        if query_kind == 'metadata':
            ...
            has_next_page = True
            while has_next_page:
                response = getattr(getattr(self.youtube_metadata_client, resource.replace(
                    "()", ""))(), action)(**query_params).execute()
                query_params["pageToken"] = response.get("nextPageToken", "")
                has_next_page = bool(query_params.get("pageToken", ""))
                self.state["units_consumed"] += query_params.get(
                    "maxResults", 1)
                yield response['items']
        if query_kind == 'transcript':
            try:
                response = getattr(
                    self.youtube_transcript_client, query)(**query_params)
                time.sleep(8)
                yield response
            except:
    ...
    def _extract_and_add_as_column(self, df: pd.DataFrame, extract_dict: dict, clean_up: bool) -> pd.DataFrame:
        ...
        assert len(extract_dict.keys()) == 2 and extract_dict.keys() == {
            'extract', 'from'}, "Passed dict should have only two fields: ('extract', 'from')"
        assert len(extract_dict['extract']) == len(
            extract_dict['from']), "Fields to be extracted are not equal to the columns specified"
        assert pd.Series(extract_dict['from']).isin(df.columns).all(
        ...
        for extract, from_column in zip(*extract_dict.values()):
            df[extract] = df[from_column].apply(
                lambda x: x.get(extract, np.NaN))
        if clean_up:
    ...
    def _align(self, *list_of_dfs: pd.DataFrame, on: str, how: str) -> pd.DataFrame:
        ...
        """
        assert how in [
            'inner', 'outer'], f"The argument 'how' should be one of [{'inner', 'outer'}]"
    ...
    def _from_playlist_ids(self, *playlist_ids: Union[str, list]) -> pd.DataFrame:
        ...
        metadata.rename(columns={'id': 'playlistId', 'title': 'playlist_title',
                                 'description': 'playlist_description'}, inplace=True)
        return metadata
    ...
    def _from_video_ids(self, *video_ids: Union[str, list]) -> pd.DataFrame:
        ...
        metadata = self._extract_and_add_as_column(
            metadata, extract_dict, clean_up=True)
        metadata.rename(columns={'id': 'videoId', 'title': 'video_title',
                                 'description': 'video_description'}, inplace=True)
        return metadata
    ...
    def from_playlist(self, *playlists: Union[str, list]) -> pd.DataFrame:
        ...
        def extract_playlist_id(
            playlist): return playlist[playlist.find('&list=')+6:]
        playlist_ids = [extract_playlist_id(
        ...
        playlist_data_merged = self._align(
            video_data_merged, playlist_metadata, on="playlistId", how="outer").drop_duplicates(subset='videoId')
